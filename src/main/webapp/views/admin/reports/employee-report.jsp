...
document.addEventListener("DOMContentLoaded", function () {
    ...
    // Build Designation filter from actual employee data.
    ...
    employeeTable.column(7).data().each(function (value) {
        ...
        if (designation) {
            designationValues[designation] = true;
        }
    });

    Object.keys(designationValues).sort().forEach(function (designation) {

        var safeValue = $("<div>").text(designation).html();

        $("#designationFilterMenu").append(
            '<li>' +
                '<a href="javascript:void(0);" class="dropdown-item rounded-1 designation-filter" data-value="' +
                safeValue + '">' +
                safeValue +
...
<script>
/* Employee Report year filter + dynamic chart */
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (!table || !chartElement || typeof ApexCharts === "undefined") return;

    function getRows() {
        var result = [];
        if (window.jQuery && jQuery.fn.DataTable && jQuery.fn.DataTable.isDataTable(table)) {
            var dt = jQuery(table).DataTable();
            dt.rows().every(function () { result.push(this.node()); });
        } else {
            table.querySelectorAll("tbody tr").forEach(function (r) { result.push(r); });
        }
        return result;
    }
    ...
    if (yearMenu) {
        yearList.forEach(function (year) {
            ...
            yearMenu.insertAdjacentHTML("beforeend", '<li><a href="javascript:void(0);" class="dropdown-item rounded-1 employee-year-filter" data-year="' + year + '">' + year + '</a></li>');
        });
    }
    ...
    var chart = new ApexCharts(chartElement, {
        ...
        xaxis: { categories: ["Active Employees", "Inactive Employees"] },
    ...
    function updateChart(year) {
        ...
        rows.forEach(function (row) {
            if (rowYear(row) !== year) return;
            if (rowStatus(row) === "active") active++;
            else if (rowStatus(row) === "inactive") inactive++;
        });
        chart.updateSeries([{ name: "Employees", data: [active, inactive] }]);
    }

    updateChart(years[currentYear] ? currentYear : (yearList[0] || currentYear));

    document.addEventListener("click", function (e) {
        var item = e.target.closest(".employee-year-filter");
        if (!item) return;
        e.preventDefault();
        var value = item.getAttribute("data-year");
        var year = value === "current" ? currentYear : parseInt(value, 10);
        if (yearButton) yearButton.textContent = value === "current" ? "This Year" : String(year);
        updateChart(year);
    });
});
</script>
