                   ...
                   <div class="mb-2">
                      <div class="dropdown">
                         ...
                         <ul class="dropdown-menu  dropdown-menu-end p-3">
                            <li>
                               <a href="javascript:void(0);" id="exportAttendancePdf" class="dropdown-item rounded-1"><i class="ti ti-file-type-pdf me-1"></i>Export as PDF</a>
                            </li>
                            <li>
                               <a href="javascript:void(0);" id="exportAttendanceExcel" class="dropdown-item rounded-1"><i class="ti ti-file-type-xls me-1"></i>Export as Excel </a>
                            </li>
                         </ul>
                      </div>
                ...
                <div class="col-xl-6">
                      ...
                      <div class="card-header border-0 pb-0">
                         <div class="d-flex flex-wrap justify-content-between align-items-center">
                            ...
                            <div class="dropdown">
                               <a href="javascript:void(0);" id="attendanceYearBtn" class="dropdown-toggle btn btn-sm fs-12 btn-white d-inline-flex align-items-center" data-bs-toggle="dropdown">
                                  This Year
                               </a>
             ...
             <div class="card">
                <div class="card-header d-flex align-items-center justify-content-between flex-wrap row-gap-3">
                   ...
                   <div class="d-flex my-xl-auto right-content align-items-center flex-wrap row-gap-3">
                         ...
                         <a href="javascript:void(0);" id="attendanceStatusBtn" class="dropdown-toggle btn btn-white d-inline-flex align-items-center" data-bs-toggle="dropdown">
                            Select Status
                         </a>
                         <ul class="dropdown-menu dropdown-menu-end p-3" id="attendanceStatusMenu">
                            <li><a href="javascript:void(0);" class="dropdown-item rounded-1 attendance-status-option" data-status="">All Status</a></li>
                            <li><a href="javascript:void(0);" class="dropdown-item rounded-1 attendance-status-option" data-status="Present">Present</a></li>
                            <li><a href="javascript:void(0);" class="dropdown-item rounded-1 attendance-status-option" data-status="Absent">Absent</a></li>
                            <li><a href="javascript:void(0);" class="dropdown-item rounded-1 attendance-status-option" data-status="Half Day">Half Day</a></li>
                            <li><a href="javascript:void(0);" class="dropdown-item rounded-1 attendance-status-option" data-status="Leave">Leave</a></li>
                            <li><a href="javascript:void(0);" class="dropdown-item rounded-1 attendance-status-option" data-status="Holiday">Holiday</a></li>
                         </ul>
                      </div>
                            <div class="dropdown">
                         <a href="javascript:void(0);" id="attendanceSortBtn" class="dropdown-toggle btn btn-white d-inline-flex align-items-center" data-bs-toggle="dropdown">
                            Sort By : Last 7 Days
                         </a>
                         <ul class="dropdown-menu dropdown-menu-end p-3" id="attendanceSortMenu">
                            <li><a href="javascript:void(0);" class="dropdown-item rounded-1 attendance-sort-option" data-sort="recent">Recently Added</a></li>
                            <li><a href="javascript:void(0);" class="dropdown-item rounded-1 attendance-sort-option" data-sort="asc">Ascending</a></li>
                            <li><a href="javascript:void(0);" class="dropdown-item rounded-1 attendance-sort-option" data-sort="desc">Descending</a></li>
                            <li><a href="javascript:void(0);" class="dropdown-item rounded-1 attendance-sort-option" data-sort="month">Last Month</a></li>
                            <li><a href="javascript:void(0);" class="dropdown-item rounded-1 attendance-sort-option" data-sort="7days">Last 7 Days</a></li>
                         </ul>
                      </div>
                   </div>
                </div>
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (attendanceChartElement && typeof ApexCharts !== "undefined") {
        ...
        new ApexCharts(attendanceChartElement, options).render();
    }
    ...
    /* DataTables custom filtering for status/date/sort windows. */
    if (dt) {
        jQuery.fn.dataTable.ext.search.push(function (settings, data) {
            if (settings.nTable !== table) return true;

            var date = parseDate(data[1]);
            var status = (data[3] || "").trim();

            if (selectedStatus && status !== selectedStatus) return false;
            if (selectedStart && (!date || date < selectedStart)) return false;
            if (selectedEnd && (!date || date > selectedEnd)) return false;
            return true;
        });
    }
    ...
    /* ---------- Status filter ---------- */
    document.querySelectorAll(".attendance-status-option").forEach(function (item) {
        item.addEventListener("click", function (e) {
            e.preventDefault();
            selectedStatus = this.getAttribute("data-status") || "";
            var btn = document.getElementById("attendanceStatusBtn");
            if (btn) btn.innerText = selectedStatus || "Select Status";
            redraw();
        });
    });

    /* ---------- Date range filter ---------- */
    var dateInput = document.getElementById("attendanceDateRange");
    if (dateInput && window.jQuery && jQuery.fn.daterangepicker) {
        ...
        jQuery(dateInput).on("apply.daterangepicker", function (ev, picker) {
            selectedStart = picker.startDate.clone().startOf("day").toDate();
            selectedEnd = picker.endDate.clone().endOf("day").toDate();
    ...
    /* ---------- Sort / date-window filter ---------- */
    document.querySelectorAll(".attendance-sort-option").forEach(function (item) {
        item.addEventListener("click", function (e) {
            e.preventDefault();
            var sort = this.getAttribute("data-sort");
            var label = this.textContent.trim();
            var btn = document.getElementById("attendanceSortBtn");
            if (btn) btn.innerText = "Sort By : " + label;
            ...
            var now = new Date();
            if (sort === "month") {
                selectedStart = new Date(now.getFullYear(), now.getMonth() - 1, now.getDate());
                selectedEnd = new Date();
            } else if (sort === "7days") {
                selectedStart = new Date();
                selectedStart.setDate(selectedStart.getDate() - 7);
                selectedEnd = new Date();
            }

            if (dt) {
                if (sort === "asc") dt.order([1, "asc"]).draw();
                else dt.order([1, "desc"]).draw();
            } else {
                redraw();
            }
        });
    ...
    /* ---------- Year selector / chart year filter ---------- */
    var yearMenu = document.getElementById("attendanceYearMenu");
    var yearBtn = document.getElementById("attendanceYearBtn");
    if (yearMenu && yearBtn) {
        var years = new Set();
        getRows().forEach(function (row) {
            var cells = row.querySelectorAll("td");
            var date = cells[1] ? parseDate(cells[1].innerText) : null;
        ...
        Array.from(years).sort(function(a,b){ return b-a; }).forEach(function (year) {
            ...
            a.addEventListener("click", function (e) {
                e.preventDefault();
                yearBtn.innerText = String(year);
                selectedStart = new Date(year, 0, 1);
                selectedEnd = new Date(year, 11, 31, 23, 59, 59);
                if (dateInput) dateInput.value = "01/01/" + year + " - 31/12/" + year;
                redraw();
            });
            li.appendChild(a);
    ...
    function getVisibleRowsForExport() {
        if (dt) {
            return dt.rows({ search: "applied", order: "applied" }).nodes().toArray();
        }
        return getRows().filter(function (row) { return row.style.display !== "none"; });
    }

    var excelBtn = document.getElementById("exportAttendanceExcel");
    if (excelBtn) {
        excelBtn.addEventListener("click", function (e) {
            e.preventDefault();
            var headers = Array.from(table.querySelectorAll("thead th")).map(function (th) { return th.innerText.trim(); });
            var lines = [headers.map(csvEscape).join(",")];
            getVisibleRowsForExport().forEach(function (row) {
                var cells = Array.from(row.querySelectorAll("td")).map(function (td) { return csvEscape(td.innerText); });
    ...
    /* ---------- PDF: browser print dialog, user can choose Save as PDF ---------- */
    var pdfBtn = document.getElementById("exportAttendancePdf");
    if (pdfBtn) {
